...
class FakeBalanceSource:
    ...
    def __init__(
        self, balances: dict[tuple[str, str, str], Decimal] | None = None
    ) -> None:
        self._funds: dict[tuple[str, str, str], PoolFunds] = {
            pool: PoolFunds(total=amount, available=amount)
            for pool, amount in (balances or {}).items()
        }

    def set_balance(
        self, exchange: str, venue: str, settlement_currency: str, balance: Decimal
    ) -> None:
        """A pool with nothing committed: its total and availability agree."""
        self.set_funds(
            exchange, venue, settlement_currency, total=balance, available=balance
        )
    ...
    def set_funds(
        ...
        *,
        ...
        self._funds[(exchange, venue, settlement_currency)] = PoolFunds(
    ...
    async def read_balance(
        ...
        try:
            return self._funds[(exchange, venue, settlement_currency)]
        except KeyError as exc:
            raise InvariantViolation(
                f"no balance configured for ({exchange}, {venue}, {settlement_currency})"
            ) from exc
