"""Fixtures for integration tests that hit a real PostgreSQL database.

Uses ``strategy_manager_test``, never the dev database (``strategy_manager``).
A session/connection is opened per test to give each test its own physical
connection — required by the crash-reclaim and concurrency tests, which
depend on distinct connections observing each other's locks and rollbacks.
"""
