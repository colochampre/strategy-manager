...
def test_pool_config_rejects_zero_min_order_size() -> None:
    with pytest.raises(InvariantViolation):
        PoolConfig(
            venue=Venue.SPOT, settlement_currency=Currency.USDT, min_order_size=Decimal("0")
        )
...
def test_pool_config_rejects_negative_min_order_size() -> None:
    with pytest.raises(InvariantViolation):
        PoolConfig(
            venue=Venue.SPOT, settlement_currency=Currency.USDT, min_order_size=Decimal("-5")
        )
