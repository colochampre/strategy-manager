...
Extended with ``strategies``, ``reservations`` and ``execution_attempts``
(mirroring ``tests/ledger/infrastructure/conftest.py``) for
``InFlightWorkAdapter``'s integration test (design.md § "In flight vs
orphan"): proving the join reaches a strategy through whichever of a
reservation's two attempt kinds is set is a real SQL question, not a fake
one.
"""
