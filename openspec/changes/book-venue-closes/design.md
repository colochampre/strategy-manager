...
**Binance**: `GET /fapi/v1/userTrades` + `symbol`, `startTime`, `endTime`, `limit`; on a full page, advance `startTime` to the last trade's time + 1ms and de-duplicate by `exchange_fill_id`, same `max_pages` bound. Provisional on probe item (b), because `fromId` and a time range may not combine.
